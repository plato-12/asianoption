...
calc_probabilities <- function(u, d, r, n) {
  ...
  calc_pn_u <- function(u, d, r, n) {
    numerator <- 1 - (((u^n - 1) / (u - 1)) - (n / (n - 1)) * r * ((u^(n - 1) - 1) / (u - 1)))
...
find_valid_ranges <- function(n, u_range = c(1, 1.5), d_range = c(0.5, 1), r_range = c(0.9, 1.2), step = 0.01) {
  ...
  for (u in seq(u_range[1], u_range[2], by = step)) {
    for (d in seq(d_range[1], d_range[2], by = step)) {
      for (r in seq(r_range[1], r_range[2], by = step)) {
        # Check if u * d = 1 (product condition)
        if (abs(u * d - 1) > 1e-6) next  # Allow a small tolerance
...
# Example usage
n <- 3  # Example number of time steps
