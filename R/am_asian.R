...
#' Calculate Risk-Neutral Probability for Up Movement
#'
#' This function calculates the risk-neutral probability of an up movement
#' (\eqn{p_n^u}) in a binomial tree at step \code{n}.
#'
...
#' @export
#'
#' @examples am_pn_u(u = 1.01, d = 0.54, r = 0.90, n = 5)
...
#' Calculate Risk-Neutral Probability for Down Movement
#'
#' This function calculates the risk-neutral probability of a down movement
#' (\eqn{p_n^d}) in a binomial tree at step \code{n}.
#'
#' @param u Numeric. The up factor representing the relative increase in the stock price.
#' @param d Numeric. The down factor representing the relative decrease in the stock price.
#' @param r Numeric. The risk-free rate (expressed as a factor, e.g., 1.05 for 5%).
...
#'
#' @examples
#' am_pn_d(u = 1.01, d = 0.54, r = 0.90, n = 5)
...
#'
#' This function prices an Asian call option using a binomial tree model.
#' It calculates all possible stock price paths, average prices along each path,
...
#' @param K Numeric. The strike price of the Asian call option.
#'
#' @return A list containing:
#' \describe{
#'   \item{price_paths}{Matrix. All possible stock price paths.}
#'   \item{prob_paths}{Matrix. Path probabilities at each step.}
#'   \item{avg_prices}{Numeric vector. Average prices along each path.}
#'   \item{payoffs}{Numeric vector. Option payoffs for each path (\eqn{\max(0, \text{avg\_price} - K)}).}
#'   \item{final_probabilities}{Numeric vector. The final probability for each path.}
#'   \item{asian_call_price}{Numeric. The price of the Asian call option.}
#' }
#' @export
#'
#' @examples
...
#' K <- 100     # Strike price
#'
#' result <- cal_am_asian(S0, u, d, r, n, K)
#' cat("The price of the Asian Call Option is:", result$asian_call_price, "\n")
cal_am_asian <- function(S0, u, d, r, n, K) {
  # Function body
}
...
cal_am_asian <- function(S0, u, d, r, n, K) {
  ...
  for (i in 1:n_paths) {
    ...
    for (j in 1:n) {
      ...
      if (path[j] == 1) {
        ...
        if (j == 1) {
          ...
          if (path[j - 1] == 1) {
            prob_paths[i, j + 1] <- am_pn_u(u, d, r, j)
          } else {
            prob_paths[i, j + 1] <- am_pn_d(u, d, r, j)
          }
        }
      }
        ...
        if (j == 1) {
          ...
          if (path[j - 1] == 1) {
            prob_paths[i, j + 1] <- 1 - am_pn_u(u, d, r, j)
          } else {
            prob_paths[i, j + 1] <- 1 - am_pn_d(u, d, r, j)
          }
        }
      }
    }
    ...
    # Compute the average price along this path
    avg_prices[i] <- mean(price_paths[i, 2:(n + 1)])
  ...
  list(price_paths = price_paths, prob_paths = prob_paths,
       avg_prices = avg_prices, payoffs = payoffs,
...
data <- cal_am_asian(S0, u, d, r, n, K)
